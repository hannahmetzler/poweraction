---
title: "Contractive pose effect on avoidance: Effect of stimulus gender"
...
* Reviewer request: Does the effect of contractive postures on avoidance of anger occur only for male stimuli?
* First check ANOVAs, then run LMERs if the effect exists. 
...
d_es <- data_clean %>%
  ...
  droplevels() %>% 
  mutate(session = factor(session, levels = c(1,2), labels = c("Baseline", "With pose")),
         stim_gender = factor(stim_gender, levels = c(1,2), labels = c("Male stimuli", "Female stimuli")))


#calculate propotions 
paway <- d_es %>%
  #group by subject, session, emotion, side
  group_by(subject, group, session, emotion, away, stim_gender) %>%
...
colpose <- c("magenta4", "mediumseagreen")
...
statsp <- paway %>%
  ...
  group_by(group, session, emotion, stim_gender, n)%>%
...
ggplot(data=statsp, aes(x=session, y=mean, colour=group, shape=group))+
  facet_grid(stim_gender~emotion) + #split plot per emotion
  geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width = 0.2, size = 1, position=pd)+ #CIs
  geom_line(aes(group=group), size=1, position=pd, linetype=1) + #connecting lines from session 1 to 2
  geom_point(position=pd) + #means per condition size =7
  ...
  theme_bw() +theme(text = element_text(size =s), 
                    ...
                    legend.position ="bottom")+ #legend position in plot
  #coordinates of y axis
  coord_cartesian(ylim=c(0.46,0.54))+scale_y_continuous(breaks=seq(0.46,0.54,0.02))
...
# geom_text(data=data.frame(emotion = "Anger", group="Constrictive", session="With Pose"), #part of plot, on top of which session (x position), group needed for default colour of asterisk)
